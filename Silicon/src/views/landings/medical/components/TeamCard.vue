<template>
  <div class="card card-hover border-0 bg-transparent">
    <div class="position-relative">
      <img :src="member.image" class="rounded-3" alt="Dr. Ronald Richards" />
      <div
      ...
      >
        ...
        <div class="position-relative d-flex zindex-2">
          ...
          <a
            ...
            aria-label="Twitter"
          >
            <Icon :icon="TwitterIcon"></Icon>
          </a>
        </div>
      </div>
    </div>
    <div class="card-body text-center p-3">
      <h3 class="fs-lg fw-semibold pt-1 mb-2">{{ member.name }}</h3>
      <p class="fs-sm mb-2">{{ member.expertise }}</p>
      <div class="d-flex align-items-center justify-content-center">
        <div class="text-nowrap me-1">
          <Icon
            v-for="(star, idx) in Array.from(new Array(Math.floor(member.rating ?? 0)))"
            :key="idx"
            :icon="StarIcon"
          ...
          <Icon
            v-for="(star, idx) in Array.from(new Array(5 - Math.ceil(member.rating ?? 0)))"
            :key="idx"
            :icon="StarBxIcon"
            class="bx text-muted opacity-75"
          ></Icon>
        </div>
        <span class="fs-xs text-muted">( {{ member.totalReview }} reviews)</span>
      </div>
    </div>
  </div>
</template>
