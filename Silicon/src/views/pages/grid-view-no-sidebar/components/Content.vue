<template>
  <b-container class="mt-4 mb-lg-5 pt-lg-2 pb-5">
        ...
        <b-form class="row gy-2">
          <b-col lg="5" sm="5">
            <div class="d-flex align-items-center">
              <div class="nav flex-nowrap me-sm-4 me-3">
                <router-link :to="{ name: 'pages.list-view-no-sidebar' }" class="nav-link me-2 p-0"
                  aria-label="List view">
                  <Icon :icon="ListUlIcon" class="fs-4" width="40" height="34" />
                </router-link>
                <router-link :to="{ name: 'pages.grid-view-no-sidebar' }" class="nav-link p-0 active"
          ...
          <b-col lg="7" sm="6">
            <div class="input-group">
              ...
              <Icon :icon="SearchIcon" class="position-absolute top-50 end-0 translate-middle-y me-3 zindex-5 fs-lg" />
            </div>
          </b-col>
        </b-form>
      </b-col>
    ...
    <!-- Blog grid -->
    <b-row class="row-cols-lg-3 row-cols-sm-2 row-cols-1 gy-md-4 gy-2">
      <b-col class="pb-3" v-for="(blog, idx) in blogData" :key="idx">
        <BlogCard :blog="blog" />
      </b-col>
    </b-row>
...
import Pagination from '@/views/pages/grid-view-no-sidebar/components/Pagination.vue';
import BlogCard from '@/views/pages/grid-view-no-sidebar/components/BlogCard.vue';
