<template>
  <figure class="d-flex flex-column h-100 px-2 px-sm-0 mb-0">
    ...
    <figcaption class="d-flex align-items-center ps-4 pt-4">
      <img :src="testimonial.author.image" width="48" class="rounded-circle" alt="Fannie Summers" />
      <div class="ps-3">
        <h6 class="fs-sm fw-semibold mb-0">{{ testimonial.author.name }}</h6>
        <span class="fs-xs text-muted">{{ testimonial.author.expertise }}</span>
      </div>
    </figcaption>
  </figure>
</template>
...
import type { TestimonialType } from '@/views/pages/single-course/type'
