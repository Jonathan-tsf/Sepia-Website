<template>
  <figure class="d-flex flex-column h-100 mt-4">
    ...
    <figcaption class="d-flex align-items-center pt-4">
      <img :src="testimonial.user.image" width="48" class="rounded-circle" alt="Annette Black" />
      <div class="ps-3">
        <h6 class="fs-sm fw-semibold mb-0">{{ testimonial.user.name }}</h6>
        <span class="fs-xs text-muted">{{ testimonial.user.position }}</span>
      </div>
    </figcaption>
  </figure>
</template>
...
import type { TestimonialType } from '@/views/pages/about-3/type'
