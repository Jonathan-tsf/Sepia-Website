<template>
  <article class="card h-100 border-0 shadow-sm">
    <div class="position-relative">
      <router-link
        :to="{ name: course.link.name, params: course.link.params }"
    ...
    <div class="card-body pb-3">
      <h3 class="h5 mb-2">
        <router-link :to="{ name: course.link.name, params: course.link.params }">{{
          course.title
        }}</router-link>
      </h3>
    ...
    <div class="card-footer d-flex align-items-center fs-sm text-muted py-4">
      <div class="d-flex align-items-center me-4">
        <Icon :icon="TimeIcon" class="fs-xl me-1" />
        {{ course.time }} hours
      </div>
      <div class="d-flex align-items-center">
        <Icon :icon="LikeIcon" class="fs-xl me-1" />
        {{ course.like }}
      </div>
    </div>
  </article>
